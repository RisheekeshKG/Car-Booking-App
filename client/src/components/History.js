    ...
    const fetchHistory = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/api/bookings/history?user=${username}`,
          {
            method: "GET",
            headers: {
      ...
      <div className="table-holder">
        ...
        {loading ? (
          ...
          <table className="table table-dark table-hover custom-table table-striped .table-responsive{-sm|-md|-lg|-xl}">
            <thead>
              <tr id="tr">
                <th className="custom-th" scope="col">
                  Guest Name
                </th>
                <th className="custom-th" scope="col">
                  Date
            ...
            <tbody>
              {historyData.map((item, index) => (
                <tr key={index} className="table-light">
                  ...
                  <td>
                    {item.bookedTimeSlots
                      ? `${item.bookedTimeSlots.from} - ${item.bookedTimeSlots.to}`
                      : "N/A"}
                  </td>
