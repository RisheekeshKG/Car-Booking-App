...
function Admin() {
    ...
    const [driverDetails, setDriverDetails] = useState({ driverAlloted: '', driverNumber: '' });
    ...
    const handleDecision = async (objectId, status) => {
        if (status === 'Accepted' && editingBookingId === objectId) {
            ...
            if (!driverAlloted || !driverNumber) {
                alert('Both driver name and number must be provided before accepting the booking.');
                return;
            }
            ...
            try {
                const response = await fetch('http://localhost:3000/api/admin/decision', {
                    ...
                    body: JSON.stringify({ objectId, status, driverAlloted, driverNumber }),
                });

                if (response.ok) {
                    const updatedBookings = bookings.map(booking => {
                        if (booking._id === objectId) {
                            return { ...booking, Status: status, driverAlloted, driverNumber };
                        }
                        return booking;
                    });
                            ...
                            {loading ? (
                                ...
                                bookings.map(booking => (
                                    <tr key={booking._id}>
                                        ...
                                        <td>
                                            {editingBookingId === booking._id ? (
                                                <button className="btn btn-save btn-sm" onClick={() => handleDecision(booking._id, 'Accepted')}>Save & Accept</button>
                                            ) : (
                                                <>
                                                    <button className="btn btn-accept btn-sm" onClick={() => handleEdit(booking._id)}>Assign & Accept</button>
